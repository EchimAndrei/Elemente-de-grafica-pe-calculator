...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint type = glGetUniformLocation(shader->program, "typeLight");
    glUniform1i(type, typeOfLight);

	GLint cut_off = glGetUniformLocation(shader->program, "cut_off_angle");
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
		...
		if (window->KeyHold(GLFW_KEY_PAGE_UP)) lightPosition2 += up * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_PAGE_DOWN)) lightPosition2 -= up * deltaTime * speed;
