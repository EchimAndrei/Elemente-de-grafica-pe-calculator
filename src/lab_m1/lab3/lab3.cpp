...
void Lab3::Update(float deltaTimeSeconds)
{
    ...
    scaleX = 1 + sin(angularStep) * 0.5f;
    scaleY = 1 + cos(angularStep) * 0.5f;
    ...
    // Pamantul care orbitreaza soarele
    squareSize = 100.f;
    modelMatrix *= transform2D::Translate(squareSize / 2.f, squareSize / 2.f);
    modelMatrix *= transform2D::Rotate(angularStep * 2.f);
    modelMatrix *= transform2D::Translate(-squareSize * 2.f, -squareSize * 2.f);
    RenderMesh2D(meshes["square2"], shaders["VertexColor"], modelMatrix);

    // Luna care orbiteaza Pamantul
    modelMatrix *= transform2D::Translate(squareSize / 2.f, squareSize / 2.f);
    modelMatrix *= transform2D::Rotate(angularStep * 2.f);
    modelMatrix *= transform2D::Translate(squareSize * 1.2f, squareSize * 1.2f);
    modelMatrix *= transform2D::Translate(-squareSize / 2.f, -squareSize / 2.f);
    RenderMesh2D(meshes["square3"], shaders["VertexColor"], modelMatrix);
}
